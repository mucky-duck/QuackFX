            ...
            for color_image_file in paths:
                if not xbmcvfs.exists(color_image_file):
                    if SUPPORTS_PIL:
                        ...
                        xbmcvfs.copy("https://dummyimage.com/16/%s/%s.png"
                                     % (colorstring[2:], colorstring[2:]), color_image_file)
                        log_msg("Local PIL module not available, generating color swatch image with online service",
                                xbmc.LOGWARNING)
